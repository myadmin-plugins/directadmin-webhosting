...
class Plugin
{
    ...
    public static function getActivate(GenericEvent $event)
    {
        if ($event['category'] == get_service_define('WEB_DIRECTADMIN')) {
            ...
            request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'createacct', $options, $response);
            ...
            if ($response['result'][0]['status'] == 1) {
                ...
                if (isset($options['bwlimit']) && $options['bwlimit'] != 'unlimited') {
                    ...
                    request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'limitbw', ['username' => $username, 'options' => $options['bwlimit']], $response3);
                    myadmin_log(self::$module, 'info', 'Response: '.str_replace('\n', "\n", strip_tags($response3)), __LINE__, __FILE__, self::$module, $serviceClass->getId());
                }
                if ($reseller === true) {
                    $response2 = $whm->setupreseller($username, false);
                    request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'setupreseller', ['username' => $username], $response2);
                    ...
                    request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'listacls', [], $response);
                    ...
                    if (!isset($acls['acls']['reseller'])) {
                        ...
                        request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'saveacllist', $acl, $response);
                    ...
                    myadmin_log(self::$module, 'info', str_replace('\n', "\n", json_encode($response)), __LINE__, __FILE__, self::$module, $serviceClass->getId());
                    request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'setacls', $request, $response);
