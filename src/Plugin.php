        ...
        if (in_array($event['type'], [get_service_define('WEB_DIRECTADMIN'), get_service_define('WEB_STORAGE')])) {
            ...
            try {
                ...
                */
                ...
                myadmin_log('directadmin', 'error', 'DirectAdmin activation API call threw an exception: '.$e->getMessage(), __LINE__, __FILE__, self::$module, $serviceClass->getId());
                $event['success'] = false;
                $event['status'] = 'error';
    ...
    public static function getReactivate(GenericEvent $event)
    {
        if (in_array($event['type'], [get_service_define('WEB_DIRECTADMIN'), get_service_define('WEB_STORAGE')])) {
            ...
            try {
                ...
                myadmin_log('directadmin', 'error', 'DirectAdmin reactivation API call threw an exception: '.$e->getMessage(), __LINE__, __FILE__, self::$module, $serviceClass->getId());
                $event['success'] = false;
                $event['status'] = 'error';
    ...
    public static function getDeactivate(GenericEvent $event)
    {
        if (in_array($event['type'], [get_service_define('WEB_DIRECTADMIN'), get_service_define('WEB_STORAGE')])) {
            ...
            try {
                ...
                myadmin_log('directadmin', 'error', 'DirectAdmin deactivation API call threw an exception: '.$e->getMessage(), __LINE__, __FILE__, self::$module, $serviceClass->getId());
                $event['success'] = false;
                $event['status'] = 'error';
        ...
        if (in_array($event['type'], [get_service_define('WEB_DIRECTADMIN'), get_service_define('WEB_STORAGE')])) {
            ...
            try {
                ...
                if (trim($serviceClass->getUsername()) == '') {
                    ...
                    $event['success'] = false;
                    $event['status'] = 'error';
                    $event['status_text'] = 'Termination failed'.(isset($result['text']) && trim($result['text']) != '' ? ': '.$result['text'] : '').(isset($result['details']) && trim($result['details']) != '' ? ' '.$result['details'] : '');
                    return false;
                }
            } catch (\Throwable $e) {
                myadmin_log('directadmin', 'error', 'DirectAdmin termination API call threw an exception: '.$e->getMessage(), __LINE__, __FILE__, self::$module, $serviceClass->getId());
                $event['success'] = false;
                $event['status'] = 'error';
    ...
    public static function getChangeIp(GenericEvent $event)
    {
        if (in_array($event['type'], [get_service_define('WEB_DIRECTADMIN'), get_service_define('WEB_STORAGE')])) {
            try {
                ...
                myadmin_log('directadmin', 'error', 'DirectAdmin getChangeIp API call threw an exception: '.$e->getMessage(), __LINE__, __FILE__, self::$module, $serviceClass->getId());
                $event['success'] = false;
                $event['status'] = 'error';
