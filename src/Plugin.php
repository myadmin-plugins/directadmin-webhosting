...
class Plugin
{
	...
	{
		if ($event['category'] == get_service_define('WEB_DIRECTADMIN')) {
			...
			try {
				...
				$event['success'] = false;
				myadmin_log('directadmin', 'error', $e->getMessage(), __LINE__, __FILE__, self::$module, $serviceClass->getId());
			...
			try {
				...
				$event['success'] = false;
				myadmin_log('directadmin', 'error', 'Caught Exception from initial createacct call: '.$e->getMessage(), __LINE__, __FILE__, self::$module, $serviceClass->getId());
				$event->stopPropagation();
				return;
			}
			request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'createacct', $options, $response, $serviceClass->getId());
			...
			if ($response['result'][0]['status'] == 1) {
				...
				if (isset($options['bwlimit']) && $options['bwlimit'] != 'unlimited') {
					...
					request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'limitbw', ['username' => $username, 'options' => $options['bwlimit']], $response3, $serviceClass->getId());
					myadmin_log(self::$module, 'info', 'Response: '.str_replace('\n', "\n", strip_tags($response3)), __LINE__, __FILE__, self::$module, $serviceClass->getId());
				}
				if ($reseller === true) {
					$response2 = $whm->setupreseller($username, false);
					request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'setupreseller', ['username' => $username], $response2, $serviceClass->getId());
					...
					request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'listacls', [], $response, $serviceClass->getId());
					...
					if (!isset($acls['acls']['reseller'])) {
						$acl = [
							...
							'acl-mailcheck' => 1, // Allow the reseller to access the DiretAdmin Mail Troubleshooter.
							'acl-mod-subdomains' => 1, // Allow the reseller to enable and disable subdomains.
							'acl-news' => 1, // Allow the reseller to modify DirectAdmin news.
							...
							'acl-status' => 0, // Allow the reseller to view the Service Status feature in DirectAdmin.
						...
						request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'saveacllist', $acl, $response, $serviceClass->getId());
					...
					myadmin_log(self::$module, 'info', str_replace('\n', "\n", json_encode($response)), __LINE__, __FILE__, self::$module, $serviceClass->getId());
					request_log(self::$module, $serviceClass->getCustid(), __FUNCTION__, 'directadmin', 'setacls', $request, $response, $serviceClass->getId());
	...
	public static function getTerminate(GenericEvent $event)
	{
		if ($event['type'] == get_service_define('WEB_DIRECTADMIN')) {
			...
			if (trim($serviceClass->getUsername()) != '') {
				...
				myadmin_log(self::$module, 'info', "Skipping Server Removal for {$serviceClass->getHostname()} because username is blank", __LINE__, __FILE__, self::$module, $serviceClass->getId());
			}
	...
	public static function getChangeIp(GenericEvent $event)
	{
		if ($event['category'] == get_service_define('WEB_DIRECTADMIN')) {
			...
			$response = $directadmin->editIp($serviceClass->getIp(), $event['newip']);
			if (isset($response['faultcode'])) {
				myadmin_log(self::$module, 'error', 'DirectAdmin editIp('.$serviceClass->getIp().', '.$event['newip'].') returned Fault '.$response['faultcode'].': '.$response['fault'], __LINE__, __FILE__, self::$module, $serviceClass->getId());
				$event['status'] = 'error';
	...
	public static function getMenu(GenericEvent $event)
	{
		...
		if ($GLOBALS['tf']->ima == 'admin') {
			...
			if (has_acl('module_config')) {
				...
				$menu->add_link('whminformation', 'choice=none.whm_version', '/images/whm/version.gif', _('Display DirectAdmin Version'));
	...
	public static function getSettings(GenericEvent $event)
	{
		...
		$settings->add_dropdown_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_ip', _('DirectAdmin Package Defaults - IP'), _('Enable/Disable Dedicated IP for new Sites'), $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_IP'), ['n', 'y'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_cgi', _('DirectAdmin Package Defaults - CGI'), _('Enable/Disable CGI Access'), $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_CGI'), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_frontpage', _('DirectAdmin Package Defaults - Frontpage'), _('Enable/Disable Frontpage Extensions'), $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_FRONTPAGE'), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_hasshell', _('DirectAdmin Package Defaults - Shell'), _('Enable/Disable Shell'), $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_HASSHELL'), ['0', '1'], ['No', 'Yes']);
		$settings->add_text_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_cpmod', _('DirectAdmin Package Defaults - CP Mod'), '', $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_CPMOD'));
		$settings->add_text_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_maxsql', _('DirectAdmin Package Defaults - Max SQL'), '', $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_MAXSQL'));
		$settings->add_text_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_maxpop', _('DirectAdmin Package Defaults - Max POP3'), '', $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_MAXPOP'));
		$settings->add_text_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_maxlst', _('DirectAdmin Package Defaults - Max Mailing Lists'), '', $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_MAXLST'));
		$settings->add_text_setting(self::$module, _('DirectAdmin Defaults'), 'directadmin_package_defaults_maxsub', _('DirectAdmin Package Defaults - Max Subdomains'), '', $settings->get_setting('DIRECTADMIN_PACKAGE_DEFAULTS_MAXSUB'));
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_add_pkg', _('Reseller ACL Add PKG'), _('Allow the creation of packages.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ADD_PKG') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ADD_PKG : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_add_pkg_ip', _('Reseller ACL Add PKG IP'), _('Allow the creation of packages with dedicated IPs.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ADD_PKG_IP') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ADD_PKG_IP : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_add_pkg_shell', _('Reseller ACL Add PKG Shell'), _('Allow the creation of packages with Shell access.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ADD_PKG_SHELL') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ADD_PKG_SHELL : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_all', _('Reseller ACL All'), _('All features.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALL') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALL : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_allow_addoncreate', _('Reseller ACL Allow Addoncreate'), _('Allow the creation of packages with unlimited Addon domains.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALLOW_ADDONCREATE') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALLOW_ADDONCREATE : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_allow_parkedcreate', _('Reseller ACL Allow Parkedcreate'), _('Allow the creation of packages with parked domains.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALLOW_PARKEDCREATE') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALLOW_PARKEDCREATE : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_allow_unlimited_disk_pkgs', _('Reseller ACL Allow Unlimited Disk PKGs'), _('Allow the creation of packages with Unlimited Disk space.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALLOW_UNLIMITED_DISK_PKGS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALLOW_UNLIMITED_DISK_PKGS : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_allow_unlimited_pkgs', _('Reseller ACL Allow Unlimited PKGs'), _('Allow the creation of packages with Unlimited bandwidth.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALLOW_UNLIMITED_PKGS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ALLOW_UNLIMITED_PKGS : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_clustering', _('Reseller ACL Clustering'), _('Enable Clustering.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_CLUSTERING') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_CLUSTERING : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_create_acct', _('Reseller ACL Create Acct'), _('Allow the reseller to Create a new Account.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_CREATE_ACCT') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_CREATE_ACCT : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_create_dns', _('Reseller ACL Create DNS'), _('Allow the reseller to Add DNS zones.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_CREATE_DNS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_CREATE_DNS : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_demo_setup', _('Reseller ACL Demo Setup'), _('Allow the reseller to turn an Account into a Demo Account.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_DEMO_SETUP') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_DEMO_SETUP : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_disallow_shell', _('Reseller ACL Disallow Shell'), _('Never Allow creation of Accounts with Shell access.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_DISALLOW_SHELL') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_DISALLOW_SHELL : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_edit_account', _('Reseller ACL Edit Account'), _('Allow the reseller to modify an Account.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_EDIT_ACCOUNT') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_EDIT_ACCOUNT : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_edit_dns', _('Reseller ACL Edit DNS'), _('Allow editing of DNS zones.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_EDIT_DNS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_EDIT_DNS : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_edit_mx', _('Reseller ACL Edit MX'), _('Allow editing of MX entries'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_EDIT_MX') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_EDIT_MX : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_edit_pkg', _('Reseller ACL Edit PKG'), _('Allow editing of packages.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_EDIT_PKG') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_EDIT_PKG : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_frontpage', _('Reseller ACL FrontPage'), _('Allow the reseller to install and uninstall FrontPage extensions.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_FRONTPAGE') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_FRONTPAGE : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_kill_acct', _('Reseller ACL Kill Acct'), _('Allow termination of Accounts.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_KILL_ACCT') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_KILL_ACCT : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_kill_dns', _('Reseller ACL Kill DNS'), _('Allow the reseller to remove DNS entries.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_KILL_DNS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_KILL_DNS : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_limit_bandwidth', _('Reseller ACL Limit bandwidth'), _('Allow the reseller to modify bandiwdth Limits.').' '._('Warning').': '._('This will Allow the reseller to circumvent package bandwidth Limits, if you are not using resource Limits!'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_LIMIT_BANDWIDTH') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_LIMIT_BANDWIDTH : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_list_accts', _('Reseller ACL List Accts'), _('Allow the reseller to list his or her Accounts.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_LIST_ACCTS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_LIST_ACCTS : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_mailcheck', _('Reseller ACL Mailcheck'), _('Allow the reseller to access the DirectAdmin Mail Troubleshooter.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_MAILCHECK') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_MAILCHECK : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_mod_subdomains', _('Reseller ACL Mod SubDomains'), _('Allow the reseller to enable and disable SubDomains.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_MOD_SUBDOMAINS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_MOD_SUBDOMAINS : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_news', _('Reseller ACL News'), _('Allow the reseller to modify DirectAdmin news.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_NEWS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_NEWS : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_onlyselfandglobalpkgs', _('Reseller ACL Onlyselfandglobalpkgs'), _('Prevent the creation of Accounts with packages that are neither global nor owned by this user.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ONLYSELFANDGLOBALPKGS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_ONLYSELFANDGLOBALPKGS : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_park_dns', _('Reseller ACL Park DNS'), _('Allow the reseller to park domains.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_PARK_DNS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_PARK_DNS : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_passwd', _('Reseller ACL Passwd'), _('Allow the reseller to modify Passwords.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_PASSWD') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_PASSWD : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_quota', _('Reseller ACL Quota'), _('Allow the reseller to modify Disk Quotas.').'  '._('Warning').': '._('This will Allow resellers to circumvent package Limits for Disk space, if you are not using resource Limits!'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_QUOTA') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_QUOTA : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_rearrange_accts', _('Reseller ACL Rearrange Accts'), _('Allow the reseller to rearrange Accounts.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_REARRANGE_ACCTS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_REARRANGE_ACCTS : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_res_cart', _('Reseller ACL Res Cart'), _('Allow the reseller to reset the shopping Cart.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_RES_CART') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_RES_CART : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_status', _('Reseller ACL Status'), _('Allow the reseller to view the Service Status feature in DirectAdmin.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_STATUS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_STATUS : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_resftp', _('Reseller ACL Resftp'), _('Allow the reseller to synchronize FTP Passwords.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_RESFTP') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_RESFTP : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_restart', _('Reseller ACL Restart'), _('Allow the reseller to restart services.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_RESTART') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_RESTART : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_show_bandwidth', _('Reseller ACL Show bandwidth'), _('Allow the reseller to view Accounts bandwidth usage.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_SHOW_BANDWIDTH') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_SHOW_BANDWIDTH : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_ssl', _('Reseller ACL SSL'), _('Allow the reseller to access the SSL Manager.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_SSL') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_SSL : 0), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_ssl_gencrt', _('Reseller ACL SSL gencrt'), _('Allow the reseller to access the SSL CSR/CRT generator.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_SSL_GENCRT') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_SSL_GENCRT : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_stats', _('Reseller ACL Stats'), _('Allow the reseller to view Account Statistics.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_STATS') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_STATS : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_suspend_acct', _('Reseller ACL Suspend Acct'), _('Allow the reseller to Suspend Accounts.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_SUSPEND_ACCT') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_SUSPEND_ACCT : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_dropdown_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acl_upgrade_account', _('Reseller ACL Upgrade Account'), _('Allow the reseller to upgrade and downgrade Accounts.'), (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_UPGRADE_ACCOUNT') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACL_UPGRADE_ACCOUNT : 1), ['0', '1'], ['No', 'Yes']);
		$settings->add_text_setting(self::$module, _('Reseller ACLs'), 'directadmin_package_defaults_reseller_acllist', _('DirectAdmin Package Defaults Reseller - ACL List'), '', (defined('DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACLLIST') ? DIRECTADMIN_PACKAGE_DEFAULTS_RESELLER_ACLLIST : 'reseller'));
	}
}
